...
class API(
    ...
    def build(
        ...
        if hasattr(self, "cart"):
            cookie_l.append("cart=%s" % self.cart)
    ...
    def graphql(self, query_data):
        ...
        contents = self.post(
            url=url, data=query_data, headers={"Content-Type": "application/graphql"}
        )
        return contents
    ...
    def verify_request(self, request, field="hmac", header="X-Shopify-Hmac-SHA256"):
        ...
        if is_param:
            params_l = [
                (key, param)
                for key, param in appier.legacy.iteritems(request.get_params())
                if not key == field
            ]
            params_l.sort()
    ...
    def verify_signature(self, signature, data, key=None, base_64=True):
        ...
        appier.verify(
            key, message="No key for signing found", exception=appier.OperationalError
        )
...
class OAuthAPI(appier.OAuth2API, API):
    ...
    def oauth_authorize(self, state=None):
        ...
        if self.access_mode:
            values["grant_options[]"] = self.access_mode
        if state:
            values["state"] = state
    ...
    def _fetch_many(
        ...
        **kwargs
        ...
        while item_remaining > 0:
            contents = self.get(url, limit=limit, since_id=last_id, **kwargs)
